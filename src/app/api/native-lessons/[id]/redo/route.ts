...
import { makeRedoLessonUseCase } from '@/features/native-lessons/application/factories/redo-lesson.factory';
...
export async function POST(
  ...
  try {
    ...
    const useCase = makeRedoLessonUseCase();
    const progress = await useCase.execute(user.id, id);
    return NativeLessonPresenter.redoSuccess(progress);
  } catch (error) {
