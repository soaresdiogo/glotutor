...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'evaluation_jobs_user_id_users_id_fk') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'evaluation_jobs_text_id_texts_id_fk') THEN
  ...
  END IF;
END $$;--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "ej_user_idx" ON "evaluation_jobs" USING btree ("user_id");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "ej_status_created_idx" ON "evaluation_jobs" USING btree ("status","created_at");--> statement-breakpoint
