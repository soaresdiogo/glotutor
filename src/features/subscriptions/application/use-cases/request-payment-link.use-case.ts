...
export interface IRequestPaymentLinkUseCase {
  execute(dto: RequestPaymentLinkDto): Promise<{ success: true }>;
}
...
export class RequestPaymentLinkUseCase implements IRequestPaymentLinkUseCase {
  ...
  async execute(dto: RequestPaymentLinkDto): Promise<{ success: true }> {
    ...
    await this.emailService.sendPaymentLinkEmail(dto.email, paymentLink);
